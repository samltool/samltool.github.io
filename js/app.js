(function () {
  ...
  function parseSaml(val) {
    if (!val) {
      return;
    }

    var decoded = window.decode(val).result;
    decoded = vkbeautify.xml(decoded);

    xmlEditor.setValue(decoded);

    var samlObj = x2js.xml_str2json(decoded);

    if (!samlObj) { return; }

    if (!samlObj.Assertion) { return; }
    ...
    if (samlObj.Assertion.Conditions) {
      ...
      if (date) {
          saml_info.expires = new Date(date);
      }
    }
        ...
        if (samlObj.Assertion.Signature.KeyInfo.X509Data) {
          if (samlObj.Assertion.Signature.KeyInfo.X509Data.X509Certificate) {
            ...
            if (public_key) {
              ...
              saml_info.thumbprint = shaObj.getHash("SHA-1", "HEX");
            }
          }
        }
      }
